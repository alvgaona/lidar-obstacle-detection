...
void ClusterHelper(int index, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed,
  ...
  std::vector<int> nearest = tree->Search(points[index], distance_tolerance);

  for (int id : nearest) {
    if (!processed[id]) ClusterHelper(id, points, cluster, processed, tree, distance_tolerance);
  }
}

std::vector<std::vector<int>> EuclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distance_tolerance) {
  ...
  while (i < points.size()) {
    ...
    ClusterHelper(i, points, cluster, processed, tree, distance_tolerance);
...
int main() {
  ...
  std::cout << "Clustering found " << clusters.size() << " and took " << elapsed_time.count() << " milliseconds" << std::endl;
  ...
  for (std::vector<int> cluster : clusters) {
    ...
    render.RenderPointCloud(viewer, clusterCloud, "cluster" + std::to_string(clusterId), colors[clusterId % 3]);
    ++clusterId;
  }
  if (clusters.size() == 0) {
